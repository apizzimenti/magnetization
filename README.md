...
* `update.sh`, which sends the files _in the current experiment's directory_ to Hopper or Pangolin. For example, if you're in the `experiments/test` directory and you run `$ sh update.sh -h`, it will send all the files in the current directory to Hopper. 
...
* `simulate.py`, which (by default) contains code for simulating the plaquette invaded-cluster algorithm.
    * **By default, only the outputs from the `Chain` generator are stored and compressed. Typically, this significantly speeds up the simulation process. The chains are then "replayed" in the next step to harvest data.**
