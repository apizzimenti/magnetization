...
with open(output/"tape"/stamps[-1]/"metadata.json", "r") as f: metadata = json.load(f)
...
for data in amortized:
	...
	ax.text(
		iterations*0.65, rolling[-1]+0.006,
...
ax.set_yticklabels([rf"${t:.2f}$" for t in ax.get_yticks()], fontsize=6)
